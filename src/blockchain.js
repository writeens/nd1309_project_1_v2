...
class Blockchain {
  ...
  submitStar(address, message, signature, star) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (!isVerified) {
          throw new Error("Bitcoin Message is not verified");
        }
  ...
  validateChain() {
    ...
    let errorLog = [];
    return new Promise(async (resolve, reject) => {
      self.chain.forEach(async (block, index, arr) => {
        //AWAIT VALIDATION
        const isValidBlock = await block.validate();
        if (!isValidBlock) {
          errorLog.push(new Error("Block validation failed"));
        }
        if (index > 0 && block.previousBlockHash !== arr[index - 1].hash) {
          errorLog.push(new Error(`Chain is broken at Block ${block}`));
        }
      });
      resolve(errorLog);
    });
